...
func AddContent(c *fiber.Ctx) error {
    ...
    if content.RoomID.IsZero() || content.Type == "" || content.URL == "" {
        return c.Status(fiber.StatusBadRequest).SendString("RoomID, Type, and URL are required")
    }
    ...
    if err != nil {
        return c.Status(fiber.StatusForbidden).SendString("Room not found or you do not have permission")
    }
    ...
    if err != nil {
        return c.Status(fiber.StatusInternalServerError).SendString("Failed to add content to room")
    }
